...
def test_contents_indexes():
  actual = check_into_br1.contents_indexes('../tests/search-files/links.html')
  ...
  actual = check_into_br1.contents_indexes('../tests/search-files/recipe.html')
  expected = [('serve done',),]
  assert expected == actual, 'contents_indexes() failed; expected:\n' + str(expected) + '\nactual:\n' +str(actual)
...
def test_save_search_labels(file_paths):
  ...
  actual = check_into_br1.msg
  assert check_into_br1.msg == expected, 'test_save_search_labels() failed; actual:\n' + actual + '\nexpected:\n' + expected
