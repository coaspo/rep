...
from quz.model import quiz_categories
...
CONFIG = {'LOG_DIR': TMP_DIR, 'LOG_LEVEL': 'CRITICAL', 'QUIZZES_DIR': TMP_DIR, 'LATEST_QUIZ_CATEGORY': 'quiz'}
set_logger(CONFIG)
...
def test_no_latest_work_file():
    ...
    with open(TMP_DIR + r'\java-.3.json', 'w') as f:
        f.write('dummy content')
    latest_category, categories = quiz_categories(TMP_DIR)
...
def test_with_latest_work_file():
    with open(TMP_DIR + r'\scrum-agile.1.json', 'w') as f:
        f.write('dummy content')
    latest_work = {'LATEST_QUIZ_CATEGORY': 'quiz'}
